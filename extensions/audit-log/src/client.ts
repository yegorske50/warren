/**
 * Minimal HTTP client for the two warren surfaces the collector needs:
 * `GET /runs` (discovery) and `GET /runs/:id/events` (the NDJSON tail).
 *
...
/**
 * One page of the runs list. The server paginates by `?limit`/`?offset`
 * (limit capped at 500 server-side) over a list sorted `started desc`.
 *
 ...
 * implies `follow=false` — the response closes after the page, which is
 * exactly the at-least-once-friendly read the collector wants: no held
 * connection, no half-consumed stream to reason about on restart.
 *
 * FRICTION §1: the alternative, `?follow=1`, holds one long-lived
 * connection per active run (linear fan-out, one failure domain per
 * connection). The endpoint has no long-poll mode ("block until new
 * events or a timeout"), so the collector chooses bounded polling and
 * pays poll-interval latency instead of connection fan-out.
 */
